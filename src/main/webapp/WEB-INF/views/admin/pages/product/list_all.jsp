                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    <div class="col-lg-3 col-xl-2">
                                        <a href="${rootAdmin}product/init-insert" class="btn btn-primary mb-3 mb-lg-0"><i class='bx bxs-plus-square'></i>New Product</a>
                                    </div>
                                    <div class="col-lg-9 col-xl-10">
                                        <form class="float-lg-end" method="post" action="${rootAdmin}product/search">
                                            <div class="row row-cols-lg-2 row-cols-xl-auto g-2">
                                                <div class="col">
                                                    <div class="position-relative">
                                                        <input type="text" name="key" class="form-control ps-5" placeholder="Search Any Product..."> <span class="position-absolute top-50 product-show translate-middle-y"><i class="bx bx-search"></i></span>
                                                    </div>
                                                </div>
                                                <div class="col">
                        ...
                        <div class="card">
                                    ...
                                    <tbody>
                                        <c:forEach items="${products}" var="item" varStatus="loop">
                                            <tr>
                                                ...
                                                <td><a href="${rootAdmin}/product/detail?id=${item.id}" title="Click here to view detail...">${item.name}</a> </td>
                                                <td><img src="<c:url value="/public/assets/img/product"/>/${item.image}" width="80" height="120" alt="product's img"></td>
                                                <td>${item.title}</td>
                                                <td>$${item.price}-$${item.sale_price}</td>
                                                <td>${item.status==true?"Active":"In active"}</td>
                                                <td>${item.category.name}</td>
                                                <td>
                                                    <a href="${rootAdmin}product/init-update?id=${item.id}"><i class="bx bxs-edit"></i></a>
                                                    <a href="${rootAdmin}product/delete?id=${item.id}"><i class="bx bx-trash-alt"></i></a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                            ...
                            <nav aria-label="Page navigation example" class="align-self-center">
                                <ul class="pagination round-pagination">
                                    <li class="page-item" >
                                        <a class="page-link" href="${rootAdmin}product/page/${page-1}">Previous</a>
                                    </li>
                                    <c:forEach begin="1" end="${pages}" var="page">
                                        <li class="page-item"><a class="page-link" href="${rootAdmin}product/page/${page}">${page}</a>
                                        </li>
                                    </c:forEach>
                                    <li class="page-item"><a class="page-link" href="${rootAdmin}product/page/${page+1}">Next</a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
